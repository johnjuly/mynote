...
!!! example
    ...
    * find drinkers whose phone has prefix 555.
...
* unknown value: e.g.j的吧一定有地址，只是我们不知道是啥
...
* 任意值与null相比，真值为*unknown*
* 什么时候查询才会出结果？当where从句中真值为**true**（不是 false or unknown）时
...
* joe's bar不会出现在结果中，尽管where是重言式（tautology）
...
* meaning: 一个查询结果可以用在另一个查询的where 语句中
